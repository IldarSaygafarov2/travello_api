...
class Event(models.Model):
    ...
    slug = models.SlugField(
        verbose_name="Слаг",
        help_text="Данное поле заполнять не нужно, оно заполняется автоматически",
        blank=True,
    )
    short_description = models.TextField(verbose_name="Краткое описание")
    full_description = models.TextField(verbose_name="Полное описание")
    preview = models.ImageField(
        verbose_name="Фото", upload_to=event_images_upload_to, blank=True
    )
    price = models.PositiveSmallIntegerField(verbose_name="Цена")
    corporate_client_price = models.PositiveSmallIntegerField(
        default=0, verbose_name="Цена для корпоративных клиентов"
    )
    country_from = models.CharField(
        verbose_name="Страна вылета", max_length=100, default="Ташкент"
    )
    ...
    gathering_place = models.CharField(
        verbose_name="Место сбора группы", max_length=100
    )
...
class EventGallery(models.Model):
    ...
    image = models.ImageField(
        verbose_name="Фото",
        upload_to=event_images_gallery_upload_to,
        null=True,
        blank=True,
    )
...
class EventTourProgram(models.Model):
    ...
    day = models.IntegerField(
        verbose_name="День", help_text='День тура, например: "1 день, 2 день"'
    )
    description = RichTextField(verbose_name="Описание")
    event = models.ForeignKey(
        Event, on_delete=models.CASCADE, related_name="tour_program"
    )
...
class EventTourProgramGallery(models.Model):
    event_tour_program = models.ForeignKey(
        EventTourProgram, on_delete=models.CASCADE, related_name="tour_program_gallery"
    )
    image = models.ImageField(
        verbose_name="Фото",
    )
...
class EventPriceNotIncluded(models.Model):
    event = models.ForeignKey(
        Event,
        on_delete=models.CASCADE,
        related_name="price_not_included",
        verbose_name="Тур",
    )
    title = models.CharField(max_length=200, verbose_name="Текст")
